...
///////////////////////////////////////////////   Add Record  ///////////////////////////////////////////////////////
    ...
    $("#sku").on('blur', function() {
       if($("#sku").val() != "" && new RegExp('^([A-Z]{3}[-]{1}[0-9]{3})$').test($("#sku").val()) == true) {
         param = "sku="+$("#sku").val();
        $.post("http://jadran.sdsu.edu/cgi-bin/jadrn018/exist.cgi", param, exist_handler);
           }
        });
...
|| $("#retail").val() != "") {
    ...
    $('#tabs-1').on('click', '#addbtn', function(e){
	    ...
	    form_data.append("image", document.getElementById("image").files[0]);
           $.ajax({
            url: "http://jadran.sdsu.edu/cgi-bin/jadrn018/ajax_upload.cgi",
            type: "post",
            data: form_data,
            processData: false,
            contentType: false,
          ...
          "&retail="+$("#retail").val()+
          "&image="+img_name;
              $.post("http://jadran.sdsu.edu/cgi-bin/jadrn018/add.cgi", param, add_data_handler);
            },
     ...
     $('#form').on('click', '#logoutbtn', function(){
        if(this.id == 'logoutbtn'){
              $.get("http://jadran.sdsu.edu/cgi-bin/jadrn018/logout.cgi",app_handler);           
            }  
         });
...
////////////////////////////////////////////////     Edit Record   ///////////////////////////////////////////////////////
    ...
    $("#sku2").on('blur', function() {
       if($("#sku2").val() != "" && new RegExp('^([A-Z]{3}[-]{1}[0-9]{3})$').test($("#sku2").val()) == true) {
         param = "sku="+$("#sku2").val();
        $.post("http://jadran.sdsu.edu/cgi-bin/jadrn018/exist.cgi", param, exist2_handler);
           }
        });
...
|| $("#retail2").val() != "") {
    ...
    $('#tabs-2').on('click', '#editbtn', function(e){
        ...
        { img_name = $("#hide_image2").val().toLowerCase();
          ...
          "&retail="+$("#retail2").val()+
          "&image="+img_name;
        $.post("http://jadran.sdsu.edu/cgi-bin/jadrn018/edit.cgi", param, edit_data_handler);
	...
	var form_data = new FormData($('form-2')[0]);
      ...
      $('#tabs-2').on('blur', '#sku2', function(e){
                ...
                param = "sku="+$("#sku2").val();
                $.get("http://jadran.sdsu.edu/cgi-bin/jadrn018/fill.cgi", param, handle_mod_data);
        }
        });
...
////////////////////////////////////////////////     Delete  Record   ///////////////////////////////////////////////////////
    ...
    $("#sku3").on('blur', function() {
       if($("#sku3").val() != "" && new RegExp('^([A-Z]{3}[-]{1}[0-9]{3})$').test($("#sku3").val()) == true) {
         param = "sku="+$("#sku3").val();
        $.post("http://jadran.sdsu.edu/cgi-bin/jadrn018/exist.cgi", param, exist3_handler);
           }
        });
     ...
     $('#tabs-3').on('blur', '#sku3', function(e){
 	  if($("#sku3").val() != "" && new RegExp('^([A-Z]{3}[-]{1}[0-9]{3})$').test($("#sku3").val()) == true) {
		e.preventDefault();
	        ...
	        $("#message_line3").text("");
		param = "sku="+$("#sku3").val();
	    	$.get("http://jadran.sdsu.edu/cgi-bin/jadrn018/fill.cgi", param, handle_del_data);}
	});
	...
	else
	{
	    e.preventDefault();
	    param = "sku="+$("#sku3").val();
	   $.post("http://jadran.sdsu.edu/cgi-bin/jadrn018/del.cgi", param, del_data_handler);
	}	
	});
});
...
function receive_file2(img) {
      var toDisplay = "<img src=\"/~jadrn018/proj1/images/" + img.toLowerCase() + "\" margin-left=\"250px\" height=\"100px\" width=\"150px\" />";
      $('#pic2').html(toDisplay);
}
